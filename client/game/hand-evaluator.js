...
export const HAND_RANKS = { //hand values for reference
    HIGH_CARD: 0, 
    PAIR: 1,
    TWO_PAIR: 2, 
    THREE_OF_A_KIND: 3,
...
function eval5(cards) { //evaluate a 5 card hand; returns an [hand_score, kicker] pair
    ...
    const groups = Object.entries(count).sort(([rankA, countA], [rankB, countB]) => countB - countA || rankB - rankA).map(([rank]) => Number(rank)) /*sorts cards
...
function combinations(cards) { //function to get all the 5 card combinations from 6 or 7 card hands; returns a 2D array
    const result = []
...
export function compareScores(hand1, hand2) { //compares two hands; returns a positive number if hand1 wins, a negative number if hand2 wins, and 0 if its a tie
...
export function evaluate(cards) { //returns the best hand from 5 cards
    if (cards.length === 5) {
        return eval5(cards)
    }
    return combinations(cards).map(eval5).reduce((best, curr) => compareScores(curr, best) > 0 ? curr : best)
}

export function rankHands(HoleCards, CommunityCards) { //returns a 2D array of placements ([[{index, score}], [{index, score}], etc])
