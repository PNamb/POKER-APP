...
const DIFFICULTY_THRESHOLDS = {
  ...
  random: {
    RAISE_THRESHOLD: Math.random(),
    CALL_THRESHOLD: Math.random(),
    BLUFF_CHANCE: Math.random(),
  },
};
...
function decideAction(state, playerIndex, difficulty = "medium") {
  ...
  const { canCheck, canCall, canRaise, canFold, callAmount, minRaiseAmount } =
    getLegalActions(state, playerIndex); //possible actions for the bot

  const numOpps = state.players.filter((p) => !p.folded && p !== player).length; //number of other players (besides the bot)

  const remainingDeck = (state) => {
    //returns the remaining cards in the deck, after the hole cards and community cards are accounted for
    const known = new Set([
      ...state.communityCards,
      ...state.players[playerIndex].holeCards,
    ]);
  ...
  const { RAISE_THRESHOLD, CALL_THRESHOLD, BLUFF_CHANCE } =
    DIFFICULTY_THRESHOLDS[difficulty] ?? DIFFICULTY_THRESHOLDS.medium;
  ...
  if (equity >= CALL_THRESHOLD || potOdds(state, playerIndex) < equity) {
    //call if equity >= threshold or equity > pot odds
    if (canCall) {
      return { type: "call" };
    }
    if (canCheck) {
...
export function botAction(state, playerIndex) {
  //returns an object of {type} or {type, amount} if the decides to raise
