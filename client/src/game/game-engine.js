...
function resolveWalk(state) {
  ...
  const players = state.players.map((p, i) =>
    i === winnerIndex ? { ...p, chips: p.chips + state.pot } : p
  ); //bugged
...
export function calculateSidePots(players) {
  ...
  const maxBet = Math.max(...activePlayers.map((p) => p.totalBet), 0);

  const betLevels =
    capLevels[capLevels.length - 1] === maxBet
      ? capLevels
      : [...capLevels, maxBet];
  ...
  for (const level of betLevels) {
    ...
    const payers = activePlayers.filter((p) => p.totalBet > prev);
    const amount = payers.reduce(
      (sum, p) => sum + (Math.min(level, p.totalBet) - prev),
      0
    );

    const eligiblePlayers = payers.filter(
      (p) => !p.folded && p.totalBet >= level
    );
...
export function createBotGame(playerName, numBots, options = {}) {
  //creates a bot game (no networking)
...
export function createGame(playerNames, options = {}) {
  //set state default values and initialize player values; returns a state
  const {
    smallBlind = 10,
    bigBlind = 20,
    startingChips = 500,
...
export function startHand(state) {
  ...
  players[smallBlindIndex] = postBlind(
...
export function advancePhase(state) {
  //deal community cards; returns a state
  ...
  switch (nextPhase) {
    ...
    case "showdown": {
      ...
      for (const pot of sidePots) {
        ...
        const eligible = ranked_hands.find((group) =>
        ...
        potWinners.forEach((entry, i) => {
          ...
          players = players.map((p, j) =>
            j === entry.playerIndex ? { ...p, chips: p.chips + payout } : p
          );
...
export function advanceAction(state) {
  ...
  const bigBlindPending =
    state.phase === "preflop" &&
    !state.bigBlindActed &&
    state.activeIndex === state.bigBlindIndex;
  const allActed = state.players.every(
    (p, i) => p.folded || p.chips === 0 || state.actedThisRound.includes(i)
  );
  ...
  if (!bettingComplete) {
    ...
    if (nextIndex !== null) {
      return {
        ...
        sidePots: calculateSidePots(state.players),
      };
    }
  }
...
export function fold(state, playerIndex) {
  //folds the given player and checks if all but one has folded; returns a state
  const players = state.players.map((p, i) =>
  ...
  const actives = players.filter((p) => !p.folded && p.chips + p.bet > 0);
  if (actives.length === 1) {
...
export function call(state, playerIndex) {
  //calls the current bet for the given player; returns a state
  const player = state.players[playerIndex];
...
export function raise(state, playerIndex, amount) {
  //raises by the given amount for the given player; returns a state
  const player = state.players[playerIndex];
...
export function check(state, playerIndex) {
  //checks for the given player; returns a state
...
export function applyAction(state, playerIndex, action) {
  //applies the given action for the given player; returns an action call
  validate(state, playerIndex, action);

  let next = {
    ...state,
    bigBlindActed:
      playerIndex === state.bigBlindIndex && state.phase === "preflop"
        ? true
        : state.bigBlindActed,
  };
