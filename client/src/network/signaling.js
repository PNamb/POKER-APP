...
export async function hostRoom(
  roomCode,
  { onJoinerConnected, onJoinerFailed, onJoinerDisconnected }
) {
...
async function _handleNewJoiner(
  ...
  try {
    ...
    await pc.setRemoteDescription(
      new RTCSessionDescription(JSON.parse(joinerData.offer))
    );
...
export async function joinRoom(
  roomCode,
  clientID,
  { onConnected, onFailed, onDisconnected }
) {
    ...
    unsub = onSnapshot(joinerDoc(roomCode, clientID), async (snapshot) => {
      ...
      if (data.answer && !remoteDescriptionSet) {
        ...
        try {
          ...
          console.warn(
            "[signaling] setRemoteDescription attempt failed, will retry",
            error
          );
        }
      }
