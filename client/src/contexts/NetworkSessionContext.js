...
export function NetworkSessionProvider({ children }) {
  ...
  const startHosting = useCallback(
    async ({ hostName, startingChips, bigBlind }) => {
      ...
      const closeConnection = (connectionID) => {
        ...
        try {
          ...
          console.warn(
            "[NetworkSession] can't close peer connection",
            connectionID,
            e
          );
        }
      ...
      const newHostSession = new HostSession({
        sendToConnection: (connectionID, message) => {
          ...
          if (!channel || channel.readyState !== "open") {
            console.warn(
              "[NetworkSession] no open channel for",
              connectionID,
              message
            );
            return;
      ...
      const { stop } = await hostRoom(newHostSession.roomCode, {
        onJoinerConnected: ({ clientID, channel, pc }) => {
          ...
          channel.addEventListener("message", (event) => {
            try {
              ...
              console.warn(
                "[NetworkSession] failed to parse message from",
                clientID
              );
            }
          });
        ...
        onJoinerFailed: ({ clientID, error }) => {
          console.warn(
            "[NetworkSession] joiner negotiation failed",
            clientID,
            error
          );
        },
        onJoinerDisconnected: ({ clientID, reason }) => {
          console.warn(
            "[NetworkSession] joiner disconnected",
            clientID,
            reason
          );
          channelsRef.current.delete(clientID);
  ...
  const startJoining = useCallback(async ({ playerName, roomCode }) => {
    ...
    const newClientSession = new ClientSession({
      sendToHost: (message) => {
        ...
        if (!channel || channel.readyState !== "open") {
          console.warn(
            "[NetworkSession] no open channel to host - dropping message",
            message
          );
          return;
        }
      ...
      onConnected: ({ channel, pc }) => {
        ...
        channel.addEventListener("message", (event) => {
          try {
            ...
            console.warn(
              "[NetworkSession] failed to parse message from host",
              error
            );
          }
        });
