...
export function AppProvider({ children }) {
  ...
  const [startingChips, setStartingChips] = useState(DEFAULT_SETTINGS.startingChips)
  const [startingBigBlind, setStartingBigBlind] = useState(DEFAULT_SETTINGS.startingBigBlind)
  ...
  const [sfxVolumeBeforeMute, setSfxVolumeBeforeMute] = useState(DEFAULT_SETTINGS.sfxVolume)

  const latestRef = useRef({
    musicVolume, sfxVolume, fourColorDeck, botDifficulty, hapticLevel, displayName, handsPlayed, handsWon, biggestPot, startingChips, startingBigBlind
  })
      ...
      try {
        ...
        if (raw) {
          ...
          if (saved.sfxVolume !== undefined) {
            ...
            setSfxVolumeBeforeMute(saved.sfxVolume || DEFAULT_SETTINGS.sfxVolume)
          }
          ...
          if (saved.startingChips !== undefined) setStartingChips(saved.startingChips)
          if (saved.startingBigBlind !== undefined) setStartingBigBlind(saved.startingBigBlind)
        }
      } catch (e) {
  ...
  useEffect(() => {
    if (!isLoaded) return
    persist()
  }, [fourColorDeck, botDifficulty, hapticLevel, displayName, handsPlayed, handsWon, biggestPot, startingChips, startingBigBlind, persist])
