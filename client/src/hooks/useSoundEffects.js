...
export function useSoundEffects(state) {
    ...
    const play = useCallback((key) => { //audio driver
        const p = players[key]
        if (!p) return
        p.seekTo(0)
        p.play()
    }, [players.deal, players.chip, players.fold, players.check, players.win, players.turn]) //play a sound if anything happens

    const lastActionRef = useRef(null)
    useEffect(() => { //audio-player for action buttons
    ...
    useEffect(() => { //audio-player for phase changes
        if (!state || state.phase === phaseRef.current) return
        phaseRef.current = state.phase

        if (["preflop", "flop", "turn", "river"].includes(state.phase)) play("deal")
