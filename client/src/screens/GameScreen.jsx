    ...
    return (
        <View style = {styles.container}>
            <View style = {styles.topBar}>
                <Text style = {styles.roomLabel}>{roomCode ? `Room ${roomCode}` : "Practice Table"}</Text>
            </View>

            <ScrollView horizontal contentContainerStyle = {styles.opponentRow} showsHorizontalScrollIndicator = {false}>
                {opponents.map(renderOpponent)}
            </ScrollView>

            <View style = {styles.tableCenter}>
                <PotDisplay mainPot = {state.pot} sidePots = {state.sidePots} />
                <CommunityCards cards = {state.communityCards} cardWidth = {60} style = {styles.communityCards} />
            </View>

            {state.phase === "showdown" && (
                <View style = {styles.showdownBanner}>
                    <Text style = {styles.showdownText}>
                        {state.winners?.map(w => `${state.players[w.playerIndex].name} wins ${w.amount}`).join(", ")}
                    </Text>
                    {mode === "bot" && (
