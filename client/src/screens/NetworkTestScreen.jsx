...
export default function NetworkTestScreen() {
  ...
  const { role, hostSession, session, startHosting, startJoining } = useNetworkSession();
  const [log, setLog] = useState([]);
  const [roomCodeInput, setRoomCodeInput] = useState("");

  const addLog = (line) => setLog((prev) => [...prev, `${new Date().toLocaleTimeString()}  ${line}`]);
  ...
  const handleHost = async () => {
    ...
    hs.setOnRosterChange((roster) => addLog(`Roster changed: ${JSON.stringify(roster)}`));
  };

  const handleJoin = async () => {
    if (!roomCodeInput) return addLog("Enter a room code first");
    addLog(`Joining room ${roomCodeInput}...`);
    const cs = await startJoining({ playerName: "TestJoiner", roomCode: roomCodeInput.toUpperCase() });
    cs.setOnJoinAccepted(({ roomCode }) => addLog(`JOIN_ACCEPTED! roomCode=${roomCode}`));
    cs.setOnJoinRejected(({ reason }) => addLog(`JOIN_REJECTED: ${reason}`));
    cs.setOnRosterChange((roster) => addLog(`Roster changed: ${JSON.stringify(roster)}`));
  };
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ padding: 16, gap: 12 }}>
      ...
      {hostSession && <Text style={styles.info}>roomCode: {hostSession.roomCode}</Text>}
...
const styles = StyleSheet.create({
  ...
  button: { backgroundColor: "#2b3b2b", padding: 12, borderRadius: 8, alignItems: "center" },
